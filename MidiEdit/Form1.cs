...
namespace MidiEdit
{
    public partial class Form1 : Form
    {
        ...
        static public bool onlyKeyCheck = false;
        static public bool isProgress = false;  //処理中
        ...
        public Form1()
        {
            ...
            listBox3_SelectedIndexChanged(null,null);   //言語設定
        }
        ...
        private void PlayMidiFile_Click(object sender, EventArgs e)
        {
            ...
            DebugWindows.Text = "使用鍵盤とキーチェック開始\n" + DebugWindows.Text;
            LoadMidiName.Text = midi_filename;
            //Program.MidiInfoSet(LoadMidiName.Text);

            onlyKeyCheck = true;
            SaveMidiFile_Click(null, null);
            onlyKeyCheck = false;

            DebugWindows.Text = "使用鍵盤とキーチェック完了\n" + DebugWindows.Text;
        }
                        ...
                        if (domiso_no_do == 0)
                        {
                            ...
                            if (midi_filename_pattern != null)
                            {
                                ...
                                for (int patternNum = 0; patternNum < 16/*MidiSequencer.playNoteArrayPattern.Count*/; patternNum++)
                                {
                                    ...
                                    if (isChordNoteOnly)
                                    {
                                        int wari = note_Pattern_sabun / 3;
                                        int amari = note_Pattern_sabun % 3;
                                        for(int plusmade = 0; plusmade < 3; plusmade++)
                                        {
                                            if (amari < 0)
                                            {
                                                wari--;
                                        ...
                                        note_Pattern = scaleScore[i, amari] + wari * 12;
                                        patarnPartsFind = true;
                                    }
        ...
        {
            MidiSequencer.software_type = (int)numericUpDown2.Value;
        }

        private void unload_midi_button2_Click(object sender, EventArgs e)
        {
            midi_filename_pattern = null;
            LoadMidiName2.Text = "読み込んだMIDI名２";
            DebugWindows.Text = "パターンMIDIアンロード\n" + DebugWindows.Text;
        }
        ...
        {
            languageNum = langList.SelectedIndex;

            switch (languageNum)
            {
                case 0:
                    //JP
                    label21.Text = "ベースか修正後のコードのMIDI";
                    LoadMidiFile.Text = "MIDI読み込み";
                    label4.Text = "高頻度とみなすノートの出現回数";
                    label11.Text = "生成したor読み込んだコードから型抜きするパターンのMIDI";
                    load_midi_button2.Text = "MIDI読み込み２";
                    checkBox1.Text = "修正後のCHORDのMIDIである（型抜き用）";
                    checkBox2.Text = "CHORDの音符のみを使用するパターンである（型抜き用）";
                    label5.Text = "最上段で読み込んだMIDIファイルと同じフォルダにMIDIが出力されます";
                    SaveMidiFile.Text = "生成";
                    label23.Text = "D&&Dエリア→";
                    unload_midi_button2.Text = "MIDIアンロード２";
                    label1.Text = "曲のキーと使用する鍵盤";
                    PlayMidiFile.Text = "使用鍵盤ﾁｪｯｸ";
                    label28.Text = "下の鍵盤でC4とC5の間が6〜8音程度になるように調整してください";
                    break;

                case 1:
                    //CHINA
                    label21.Text = "贝斯或修改的和弦 MIDI";
                    LoadMidiFile.Text = "MIDI 加载";
                    label4.Text = "被认为是高频的注释出现的次数";
                    label11.Text = "从生成或加载的代码中剪切的 MIDI 模式";
                    load_midi_button2.Text = "MIDI 加载 2";
                    checkBox1.Text = "这是修改后的 CHORD MIDI（用于模切）";
                    checkBox2.Text = "这是仅使用 CHORD 音符的模式（用于模切）";
                    label5.Text = "MIDI 将输出到与顶部加载的 MIDI 文件相同的文件夹中";
                    SaveMidiFile.Text = "生成";
                    label23.Text = "D&&D区域→";
                    unload_midi_button2.Text = "MIDI 卸载 2";
                    label1.Text = "使用的歌曲键和键盘";
                    PlayMidiFile.Text = "检查使用的键盘";
                    label28.Text = "请调整，使下键盘上的 C4 和 C5 之间有大约 6 至 8 个音符";
                    break;

                case 2:
                    //INDONESIA
                    label21.Text = "MIDI akord dasar atau modifikasi";
                    LoadMidiFile.Text = "PemuatanMIDI";
                    label4.Text = "Jumlah kemunculan nada yang dianggap berfrekuensi tinggi";
                    label11.Text = "Pola MIDI yang akan dipotong dari kode yang dihasilkan atau dimuat";
                    load_midi_button2.Text = "PemuatanMIDI2";
                    checkBox1.Text = "Ini adalah CHORD MIDI yang dimodifikasi (untuk die cutting)";
                    checkBox2.Text = "Ini adalah pola yang hanya menggunakan nada CHORD (untuk pemotongan mati)";
                    label5.Text = "MIDI akan di-output ke folder yang sama dengan file MIDI yang dimuat di atas";
                    SaveMidiFile.Text = "Hasilkan";
                    label23.Text = "Area D&&D→";
                    unload_midi_button2.Text = "MIDImembongkar2";
                    label1.Text = "Tombol lagu dan keyboard digunakan";
                    PlayMidiFile.Text = "Periksa keyboard yang digunakan";
                    label28.Text = "Silakan sesuaikan agar ada sekitar 6 sampai 8 nada antara C4 dan C5 di keyboard bawah";
                    break;

                case 3:
                    //ENGLISH
                    label21.Text = "MIDI of bass or modified chord";
                    LoadMidiFile.Text = "Load MIDI";
                    label4.Text = "Number of occurrences of notes considered to be high frequency";
                    label11.Text = "MIDI of pattern to be cut out from generated or loaded chord";
                    load_midi_button2.Text = "Load MIDI 2";
                    checkBox1.Text = "CHORD MIDI is fixed CHORD (for cutting out)";
                    checkBox2.Text = "Pattern using only CHORD notes (for cutting out)";
                    label5.Text = "MIDI will be output to the same folder as the MIDI file loaded at the top";
                    SaveMidiFile.Text = "Generate";
                    label23.Text = "D&&D area→";
                    unload_midi_button2.Text = "Unload MIDI 2";
                    label1.Text = "Key of song and keyboard to use";
                    PlayMidiFile.Text = "Check keyboard used";
                    label28.Text = "adjust the so that there are about 6 to 8 notes between C4 and C5, use look lower keyDisplay";
                    break;
