...
public class MidiSequencer
{
    ...
    static public int software_type = 0;
    ...
    public async Task GetMidiInfo()
    {
        ...
        while (finishedTracks != midiEvents.Tracks)
        {
            ...
            {
                IList<MidiEvent> currentTrack = midiEvents[i];
                if(currentTrack.Count < 8)
                {
                    //格納されたイベント数が少なすぎるので、多分、設定用かなんかのトラック
                    //なので次のトラックへ
                    continue;
                }
                    ...
                    for (int j = 0; j < currentTrack.Count; j++)
                    {
                        ...
                        if (messageToSend != null)
                        {
                            if (messageToSend.Type == MidiMessageType.NoteOn)
                            {
                                ...
                                if(playLengthArray[addCount] <= 0)
                                {
                                    continue;   //ノートオフイベントは追加したくない
                                }

                                //duration gateTime　はソフトによって違うらしい。。。
                                int resolution = midiEvents.DeltaTicksPerQuarterNote;   //出力ソフトごとの４分音符の長さ

                                //if (software_type == 1)
                                {
                                    playLengthArray[addCount] = playLengthArray[addCount] * 96 / resolution;
        ...
        switch (midiEvent.CommandCode)
        {
            case MidiCommandCode.NoteOn:
                {// 9n
                    ...
                    {
                        NoteEvent test = midiEvent as NoteEvent;
                        if(test.Velocity <= 0)
                        {
                            return 0;
                        }
